...
##  define hucs
hucs = "Y:\\TIGER\\reg_gp\\gp_huc6rng_gap.shp"
##  define roads
rds = "Y:\\TIGER\\reg_gp\\roads.gdb\\gp_roads"
##  define scratch space
scratch = "Y:\\TIGER\\reg_gp\\scratch"
##  define results output directory
output = "Y:\\TIGER\\reg_gp\\intDens"
...
##  removed S1640 for all hucs - used more for driveways than actual service or frontage roads
...
for huc in hucIDs:
    try:
        ...
        if huc == "130202":
            arcpy.MakeFeatureLayer_management("Y:\\TIGER\\reg_gp\\130202_rdsManuallyRemoved.shp","removeLayer")
        ...
        if huc == "130600":
            ##  remove some roads, easily identified from attributes as NUS Naval Surface Warfare Center roads
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",	'LOWER("FULLNAME")  LIKE \'forest rd%\' OR LOWER("FULLNAME")  LIKE \'fs %\' OR LOWER("FULLNAME")  LIKE \'fs %\' OR LOWER("FULLNAME")  LIKE \'forest rd%\'')

        if huc == "111401":
            ##  remove some roads, easily identified from attributes as NUS Naval Surface Warfare Center roads
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",	'LOWER("FULLNAME")  LIKE \'forest rd%\' OR LOWER("FULLNAME")  LIKE \'weyco%\' OR LOWER("FULLNAME")  LIKE \'%pvt%\' OR LOWER("FULLNAME") LIKE \'%f s%\' OR LOWER("FULLNAME") LIKE \'%unnamed%\' OR LOWER("FULLNAME") LIKE \'pr %\' OR LOWER("FULLNAME")  LIKE \'prv%\'')


        ##  copy selected roads to new dataset
...
arcpy.FeatureToRaster_conversion("P:\\Proj3\\USGap\\Vert\\Model\\data\\regions\\regclip_gp.shp","FID","Y:\\TIGER\\reg_gp\\gp_bnd","30")

##  move workspace to directory with line density outputs
arcpy.env.workspace = scratch

##  set extent and mask to region boundary grid
arcpy.env.mask = "Y:\\Avoid\\maskbuf_gp"
arcpy.env.extent = "Y:\\Avoid\\maskbuf_gp"
...
arcpy.gp.cellstatistics_sa(nulls,"Y:\\TIGER\\reg_gp\\mosaic\\gp_rddens","MAXIMUM")
