...
##  define hucs
hucs = "Y:\\TIGER\\reg_um\\um_huc6rng_gap.shp"
##  define roads
rds = "Y:\\TIGER\\reg_um\\roads.gdb\\um_roads"
##  define scratch space
scratch = "Y:\\TIGER\\reg_um\\scratch"
##  define results output directory
output = "Y:\\TIGER\\reg_um\\intDens"
...
for huc in hucIDs:
    try:
        ...
        ##  select roads by intersection with buffered huc polygon
        ...
        if huc == "040101":
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'%forest rd%\' OR LOWER("FULLNAME") LIKE \'%forest rte%\'')
        ##	
        if huc == "040202":
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'%usfs%\' OR LOWER("FULLNAME") LIKE \'%usfs%\' OR LOWER("FULLNAME") LIKE \'fs %\' OR LOWER("FULLNAME") LIKE \'%nat for%\' OR LOWER("FULLNAME") LIKE \'%pr dr%\' OR LOWER("FULLNAME") LIKE \'%pr trl%\' OR LOWER("FULLNAME") LIKE \'%prvt%\' OR LOWER("FULLNAME") LIKE \'%u s f s%\' OR LOWER("FULLNAME") LIKE \'%unnamed%\' OR LOWER("FULLNAME")  LIKE \'fs %\' OR LOWER("FULLNAME")  LIKE \'%forest rd%\' OR LOWER("FULLNAME")  LIKE \'%forest svc%\'')
        ##	
        if huc == "040301":
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'% national forest%\' OR LOWER("FULLNAME") LIKE \'%county forest%\' OR LOWER("FULLNAME") LIKE \'%f r%\' OR LOWER("FULLNAME") LIKE \'%f s%\' OR LOWER("FULLNAME") LIKE \'%fed forest%\' OR LOWER("FULLNAME") LIKE \'%federal forest%\' OR LOWER("FULLNAME") LIKE \'ff %\' OR LOWER("FULLNAME") LIKE \'%forest rd%\' OR LOWER("FULLNAME") LIKE \'%fr %\' OR LOWER("FULLNAME") LIKE \'fs %\' OR LOWER("FULLNAME") LIKE \'fs %\' OR LOWER("FULLNAME") LIKE \'%nat for%\' OR LOWER("FULLNAME") LIKE \'%national forest%\' OR LOWER("FULLNAME") LIKE \'%pr dr%\' OR LOWER("FULLNAME") LIKE \'%unknown%\' OR LOWER("FULLNAME") LIKE \'%unnamed%\' OR LOWER("FULLNAME") LIKE \'%forest service%\' OR LOWER("FULLNAME") LIKE \'fs %\' OR LOWER("FULLNAME") LIKE \'%usfs%\' OR LOWER("FULLNAME") LIKE \'%usfs%\'')
        ##	
        if huc == "110300":
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'%unnamed%\'')
        ##	
        if huc == "110701":
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'%un-named%\' OR LOWER("FULLNAME") LIKE \'%unamed%\' OR LOWER("FULLNAME")  LIKE \'%unnamed%\'')
...
arcpy.FeatureToRaster_conversion("P:\\Proj3\\USGap\\Vert\\Model\\data\\regions\\regclip_um.shp","FID","Y:\\TIGER\\reg_um\\um_bnd","30")

##  move workspace to directory with line density outputs
arcpy.env.workspace = scratch

##  set extent and mask to buffered region boundary grid
arcpy.env.mask = "Y:\\Avoid\\maskbuf_um"
arcpy.env.extent = "Y:\\Avoid\\maskbuf_um"
...
arcpy.gp.cellstatistics_sa(nulls,"Y:\\TIGER\\reg_um\\mosaic\\um_rddens","MAXIMUM")
