...
##  define hucs
hucs = "Y:\\TIGER\\reg_se\\se_huc6rng_gap.shp"
##  define roads
rds = "Y:\\TIGER\\reg_se\\roads.gdb\\se_roads"
##  define scratch space
scratch = "Y:\\TIGER\\reg_se\\scratch"
##  define results output directory
output = "Y:\\TIGER\\reg_se\\intDens"
...
for huc in hucIDs:
    try:
        ...
        if huc == "031200":
            ...
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'"FULLNAME" LIKE \'Forest Rd%\' OR "FULLNAME" LIKE \'Nfr%\' OR "FULLNAME" LIKE \'National%\'')

        if huc == "031401":
            ##  remove some roads, easily identified from attributes as forest roads or Eglin AFB roads
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'"FULLNAME" LIKE \'Dof%\' OR "FULLNAME" LIKE \'Field%\' OR FULLNAME LIKE \'Eglin%\'')

        if huc == "051202":
            ##  remove some roads, easily identified from attributes as NUS Naval Surface Warfare Center roads
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'%jeep%\' OR LOWER("FULLNAME") LIKE \'%hwy%\' OR LOWER("FULLNAME") LIKE \'%pf%\'')

        if huc == "031101":
            ##  remove some roads, easily identified from attributes as NUS Naval Surface Warfare Center roads
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'%un %\' OR LOWER("FULLNAME") LIKE \'%unm %\' OR LOWER("FULLNAME") LIKE \'%unn%\'')

        if huc == "071100":	
            arcpy.SelectLayerByAttribute_management("rdsLayer","REMOVE_FROM_SELECTION",'LOWER("FULLNAME") LIKE \'%un-named%\' OR LOWER("FULLNAME") LIKE \'%un named%\' OR LOWER("FULLNAME") LIKE \'%un rd%\' OR LOWER("FULLNAME") LIKE \'%unnamed%\' OR LOWER("FULLNAME") LIKE \'%unnmed%\'')
...
arcpy.FeatureToRaster_conversion("P:\\Proj3\\USGap\\Vert\\Model\\data\\regions\\regclip_se.shp","FID","Y:\\TIGER\\reg_se\\se_bnd","30")

##  move workspace to directory with line density outputs
arcpy.env.workspace = scratch

##  set extent and mask to buffered region boundary grid
arcpy.env.mask = "Y:\\Avoid\\maskbuf_se"
arcpy.env.extent = "Y:\\Avoid\\maskbuf_se"
...
arcpy.gp.cellstatistics_sa(nulls,"Y:\\TIGER\\reg_se\\mosaic\\se_rddens","MAXIMUM")
